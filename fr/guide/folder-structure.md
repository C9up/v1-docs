...
Les classes d'événement du module, une par cas d'usage. Chaque classe étend `BaseEvent` depuis `@c9up/pulsar/events` et déclare ses données comme propriétés publiques du constructeur.
...
Les fichiers de migration de la base de données. Exécutés par Forge (`forge db:migrate`). Chaque fichier exporte une fonction `up()` et une fonction `down()`.
