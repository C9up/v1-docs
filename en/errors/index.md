...
| `CIRCULAR_DEPENDENCY` | Circular dependency detected during resolution. Use explicit factories or decouple via Pulsar events. |
...
| Code | Description |
|------|-------------|
| `FORGE_UNKNOWN_TYPE` | Unknown generator type. Available: `service`, `entity`, `controller`, `validator`, `provider`, `migration`. |
...
| `CSRF_FAILED` | Invalid or missing CSRF token. Generate a new token and include it in the `x-csrf-token` header. |
